...
function binarize(px, w, h) {
  ...
  var W = w + 1;
  // Holds w*h*255, so Uint32 wraps at about 4100px square. The caller's 1500px cap is what keeps
  // this in range -- load-bearing for correctness, not just memory: a window sum is a difference
  // of four entries, so once they wrap independently the page binarizes to noise.
